...
% CARBONXCHNGE  Simulate the Eriksson-Welander carbon cycle model.
%
% Inputs:
%   xl0, xd0, ya0, ys0 - Initial carbon amounts in:
%                         living plants (xl), dead organic matter (xd),
%                         atmosphere (ya), and sea (ys)  [mg cm^-2]
%   N      - Number of time steps
%   dt     - Time step size [years]
%   params - Parameter vector [k1, k2, k3, k4, k5]
%              k1 = 4.3e-5  cm^2 mg^-1 yr^-1
%              k2 = 2.0e-2  yr^-1
%              k3 = 1.6e-2  yr^-1
%              k4 = arbitrary (computed from steady-state constraint)
%              k5 = 1.8e-2  yr^-1
%
% Outputs:
%   xl, xd, ya, ys - Time series of carbon in each reservoir [mg cm^-2]
%
% Model equations (Euler method):
%   dxl/dt = k1 * xl * ya - k2 * xl
%   dxd/dt = k2 * xl     - k3 * xd
%   dys/dt = k4 * (ya - k5 * ys)     [note: ya conserved via constraint]
%   ya     = a - xl - xd - ys         (conservation of total carbon)
%
% Reference:
%   Eriksson, E. & Welander, P. (1956). On a Mathematical Model of the
%   Carbon Cycle in Nature. Tellus, 8(2), 155-175.
    ...
    %% Total carbon (conserved quantity)
    a = xl0 + xd0 + ya0 + ys0;
    ...
    %% Time-march using Euler's method
    for n = 1:N
        ...
        % Enforce conservation: ya = a - xl - xd - ys
...
%% =========================================================
%%  Script section: run the model and plot results
%% =========================================================

%% --- Parameters (Eriksson & Welander, 1956) ---
k1 = 4.3e-5;   % cm^2 mg^-1 yr^-1
k2 = 2.0e-2;   % yr^-1
k3 = 1.6e-2;   % yr^-1
k5 = 1.8e-2;   % yr^-1

% k4 is arbitrary; choose so sea exchange is balanced at t=0
% At steady state: k4*(ya0 - k5*ys0) = 0  =>  ya0 = k5*ys0
% Here we simply set k4 equal to k5 (a common choice)
k4 = k5;

params = [k1, k2, k3, k4, k5];

%% --- Initial conditions (Table 1, Eriksson & Welander 1956) ---
xl0 = 400;      % Living perennial plants  [mg cm^-2]
xd0 = 500;      % Dead organic matter      [mg cm^-2]
ya0 = 460;      % Atmosphere               [mg cm^-2]
ys0 = 25000;    % Sea                      [mg cm^-2]
...
%% --- Part 2: Time-lag model ---
% Replace k2*xl with k2*s, and add: ds/dt = (xl - s) / tau
% This introduces a delayed response for living and dead plant carbon.

tau_values = [10, 50, 100];   % Years — experiment with different lags
colors = {'g', 'm', 'k'};
legend_entries = {};
...
for idx = 1:length(tau_values)
    ...
    ya_lag = zeros(1, N+1);
    ys_lag = zeros(1, N+1);
    s_lag  = zeros(1, N+1);   % lagged plant signal
    ...
    for n = 1:N
        dxl = k1 * xl_lag(n) * ya_lag(n) - k2 * s_lag(n);   % uses s, not xl
    ...
    legend_entries{end+1} = sprintf('\\tau = %d yr', tau); %#ok<SAGROW>
end
